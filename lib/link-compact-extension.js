...
function buildLinkCompactRanges(docText) {
  ...
  while (searchFrom < docText.length) {
    ...
    const bracketStart = docText.lastIndexOf("[", linkStart);
...
function getExtensionSpec() {
  ...
  function buildDecorations(ranges, linkEmoji, notelinkEmoji, imglinkEmoji) {
    const builder = new RangeSetBuilder();

    for (const { from, to, url, isImage } of ranges) {
      const emoji = isImage
        ? imglinkEmoji
  ...
  const extension = [
    plugin,
    EditorView.editorAttributes.of({ class: "link-compact-enabled" }),
  ];
