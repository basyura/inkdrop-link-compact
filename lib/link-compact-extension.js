...
function getExtensionSpec() {
  ...
  function buildDecorations(
    ...
    const labelBracket = Decoration.mark({
      class: "link-compact-label-bracket",
    });
    const urlBracket = Decoration.mark({
      class: "link-compact-url-bracket",
    });
    ...
    for (const range of ranges) {
      ...
      const { from, to, linkFrom, linkTo, labelFrom, labelTo, url, isImage } = range;
      const emoji = isImage
        ? imglinkEmoji
      ...
      decorationBuilder.add(labelFrom - 1, labelFrom, labelBracket);
      decorationBuilder.add(labelTo, labelTo + 1, labelBracket);
      decorationBuilder.add(from - 1, from, urlBracket);
      decorationBuilder.add(from, to, replacement);
      decorationBuilder.add(to, to + 1, urlBracket);
